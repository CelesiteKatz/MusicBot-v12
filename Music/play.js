  ...
  async execute(message, args, client) {
    ...
    try {
      ...
      if (serverQueue) {
        ...
        //send join message
        message.channel.send(`**🎶 Joined \`${channel.name}\` 📄 Bound to <#${message.channel.id}>**\nRequested By - ${message.author.username}#${message.author.discriminator}`)
        //if its an url
        if (urlValid) { //send searching link
          message.channel.send(`**🔍 Searching [\`LINK\`](${args.join(" ")})**`)
          //if not
        }
        else { //send searching TITLE
    ...
    //get the thumbnail
    let thumb = "https://cdn.discordapp.com/emojis/849244037788467241.png?v=1"
